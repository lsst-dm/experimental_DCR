...
class BuildDcrCoadd(GenerateTemplate):
    ...
    dcrCoadd = BuildDcrCoadd(n_step=3, input_repository="./test_data/",
                             obsids=np.arange(100, 124, 3), filter='g')
    ...
    def __init__(self, obsids=None, input_repository='.', filter='g',
        ...
        **kwargs : TYPE
        ...
        bandpass = self.load_bandpass(filter=filter, wavelength_step=wavelength_step, **kwargs)
    ...
    def calc_psf_model(self):
        ...
        """
        n_step = 1
        bandpass = self.load_bandpass(filter=self.filter, wavelength_step=None)
